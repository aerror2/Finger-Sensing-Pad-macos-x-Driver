...
#define kActionRotateL                      "ActionRotateL"
#define kActionRotateR                      "ActionRotateR"
#define kActionLaunchPad                    "Action3FDoubleClick"
...
bool ApplePS2Keyboard::init(OSDictionary * dict)
{
    ...
    parseAction("3b d, 7d d, 7d u,  3b u", _actionSwipeDown, countof(_actionSwipeDown));
    parseAction("3b d, 7b d, 7b u,  3b u", _actionSwipeLeft, countof(_actionSwipeLeft));
    parseAction("3b d, 7c d, 7c u,  3b u", _actionSwipeRight, countof(_actionSwipeRight));
    
    parseAction("3a d, 18 d, 18 u,  3a u", _actionZoomIn, countof(_actionZoomIn));
    parseAction("3a d, 1b d, 1b u,  3a u", _actionZoomOut, countof(_actionZoomOut));
    parseAction("3a d, 25 d, 25 u,  3a u", _actionRotateL, countof(_actionRotateL));
    parseAction("3a d, 0f d, 0f u,  3a u", _actionRotateR, countof(_actionRotateR));
    parseAction("3b d, 32 d, 32 u,  3b u", _actionLauchPad, countof(_actionLauchPad));
...
//REVIEW: this code needs cleanup (should be table driven like mouse/trackpad)
    ...
    str = OSDynamicCast(OSString, dict->getObject(kActionRotateL));
    if (str)
    {
        parseAction(str->getCStringNoCopy(), _actionRotateL, countof(_actionRotateL));
        setProperty(kActionRotateL, str);
    }
    
    str = OSDynamicCast(OSString, dict->getObject(kActionRotateR));
    if (str)
    {
        parseAction(str->getCStringNoCopy(), _actionRotateR, countof(_actionRotateR));
        setProperty(kActionRotateR, str);
    }
    
    str = OSDynamicCast(OSString, dict->getObject(kActionLaunchPad));
    if (str)
    {
        parseAction(str->getCStringNoCopy(), _actionLauchPad, countof(_actionLauchPad));
        setProperty(kActionLaunchPad, str);
    }
}
...
bool ApplePS2Keyboard::dispatchKeyboardEventWithPacket(UInt8* packet, UInt32 packetSize)
{
  ...
  //  IOLog("keycode translated : 0x%04x\n", adbKeyCode);
...
void ApplePS2Keyboard::receiveMessage(int message, void* data)
{
    ...
    switch (message)
    {
        ...
        case kPS2M_rotateL:
			//DEBUG_LOG("ApplePS2Keyboard: Synaptic Trackpad call Swipe Up\n");
            sendKeySequence(_actionRotateL);
            break;
        case kPS2M_rotateR:
			//DEBUG_LOG("ApplePS2Keyboard: Synaptic Trackpad call Swipe Up\n");
            sendKeySequence(_actionRotateR);
            break;
        case kPS2M_lauchPad:
			//DEBUG_LOG("ApplePS2Keyboard: Synaptic Trackpad call Swipe Up\n");
            sendKeySequence(_actionLauchPad);
            break;
